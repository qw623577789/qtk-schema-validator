...
const globalConfig = {
    number: {
        ...
        range: "自定义G:路径:{PATH} , 不在值范围内, 当前值:{VALUE},　期望:{EXPECTED}",
    },
}

describe('number', function() {
    it('typeError', function() {
        let schema = number().errorTip({
    ...
    it('enumError', function() {
        let schema = number().enum(1, 2, 3.3).errorTip({
    ...
    it('rangeError | min', function() {
        let schema = number().min(0.9).errorTip({
    ...
    it('rangeError | max', function() {
        let schema = number().max(0.9).errorTip({
            range: "自定义:路径:{PATH} , 不在值范围内, 当前值:{VALUE},　期望:{EXPECTED}"
        });
        let validator = Validator.from(schema, globalConfig);
    ...
    it('rangeError | exclusiveMin', function() {
        let schema = number().exclusiveMin(0.9).errorTip({
            range: "自定义:路径:{PATH} , 不在值范围内, 当前值:{VALUE},　期望:{EXPECTED}"
        });
        let validator = Validator.from(schema, globalConfig);
    ...
    it('rangeError | exclusiveMax', function() {
        let schema = number().exclusiveMax(0.9).errorTip({
            range: "自定义:路径:{PATH} , 不在值范围内, 当前值:{VALUE},　期望:{EXPECTED}"
        });
        let validator = Validator.from(schema, globalConfig);
        validator.validate(0.9);
